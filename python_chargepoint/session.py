...
class ChargingSession:
    ...
    def _get(self) -> None:
        ...
        self.charging_time = status.get("charging_time")
        self.energy_kwh = status.get("energy_kwh")
        self.miles_added = status.get("miles_added")
        self.miles_added_per_hour = status.get("miles_added_per_hour", 0.0)
        self.outlet_number = status.get("outlet_number")
        self.port_level = status.get("port_level")
        self.power_kw = status.get("power_kw")
        self.purpose = status.get("purpose")
        self.currency_iso_code = status.get("currency_iso_code")
        self.payment_completed = status.get("payment_completed")
        self.payment_type = status.get("payment_type")
        self.pricing_spec_id = status.get("pricing_spec_id")
        self.total_amount = status.get("total_amount")
        self.api_flag = status.get("api_flag")
        self.enable_stop_charging = status.get("enable_stop_charging")
        self.has_charging_receipt = status.get("has_charging_receipt")
        self.has_utility_info = status.get("has_utility_info")
        self.is_home_charger = status.get("is_home_charger")
        self.is_purpose_finalized = status.get("is_purpose_finalized")
        self.last_update_data_timestamp = datetime.fromtimestamp(
            status.get("last_update_data_timestamp") / 1000, tz=timezone.utc
        )
        self.stop_charge_supported = status.get("stop_charge_supported")
        self.company_id = status.get("company_id")
        self.company_name = status.get("company_name")
        self.latitude = status.get("lat")
        self.longitude = status.get("lon")
        ...
        self.update_period = status.get("update_period")
